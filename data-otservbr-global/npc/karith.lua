...
local function greetCallback(npc, creature)
    ...
    local towns = player:getStorageValue(Storage.Quest.U8_4.InServiceOfYalahar.SearoutesAroundYalahar.TownsCounter)
    if towns ~= nil and towns >= 5 then
...
if MsgContains(message, "passage") or MsgContains(message, "sail") then
    if player:getStorageValue(Storage.Quest.U8_4.InServiceOfYalahar.SearoutesAroundYalahar.TownsCounter) < 5 then
        ...
        -- Player já liberou os destinos!
        npcHandler:say("Where do you want to go? I can sail to {Ab'Dendriel}, {Darashia}, {Venore}, {Ankrahmun}, {Port Hope}, {Thais}, {Liberty Bay} or {Carlin}.", npc, creature)
        npcHandler:setTopic(playerId, 0)
        return true
    end
