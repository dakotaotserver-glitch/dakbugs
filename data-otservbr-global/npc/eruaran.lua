...
-- Versão atualizada do NPC Eruaran com todos os IDs corrigidos
-- Tabela IDS corrigida
local IDS = {
    DREAM_MATTER = 20063,
    ...
    DREAM_WARDEN_MASK = 20276,
    DREAM_WARDEN_CLAW = 20275
}
...
local REQUIRE_QUEST_PROGRESS = false -- Altere para false se quiser que o NPC funcione sem checagem de quest
local REQUIRED_QUEST_STAGE = 10     -- Valor mínimo da storage para permitir acesso

local function dreamFirst(npc, creature, message, keywords, parameters, node)
	local player = Player(creature)
	if not player then return end

	if REQUIRE_QUEST_PROGRESS and player:getStorageValue(Storage.DreamScar.Questline) < REQUIRED_QUEST_STAGE then
		npcHandler:say("You must prove yourself in the Dream realm before receiving this addon.", npc, creature)
		return
	end
		...
		if player:getStorageValue(storage + 1) < 1 then
			if player:getItemCount(20276) > 0 then
				if player:removeItem(20276, 1) then
					npcHandler:say("Here you are, enjoy your brand new addon!", npc, creature)
				...
				npcHandler:say("You do not have the required item: dream warden mask.", npc, creature)
			end
		else
			npcHandler:say("It seems you already have this addon.", npc, creature)
		end
	end
end
	...
	if player:isPremium() then
		if player:getStorageValue(storage) < 1 then
			if player:getItemCount(20275) > 0 then
				...
				npcHandler:say("You do not have the required item: dream warden claw.", npc, creature)
			end
		else
			npcHandler:say("It seems you already have this addon.", npc, creature)
		end
...
local function creatureSayCallback(npc, creature, type, message)
	...
	-- Katar ajustado para manter a mesma estrutura e estilo dos outros tipos de arma
-- Katar ajustado para manter a mesma estrutura e estilo dos outros tipos de arma

elseif MsgContains(message, "katar") and npcHandler:getTopic(playerId) == 1 then
	weapon[playerId] = TYPES.KATAR
	npcHandler:say("Do you want to create a {Crude Umbral Katar}, improve a {Crude Umbral Katar} into an {Umbral Katar}, or transform an {Umbral Katar} into an {Umbral Master Katar}?", npc, creature)
	npcHandler:setTopic(playerId, 2)

elseif MsgContains(message, "crude umbral katar") and npcHandler:getTopic(playerId) == 2 then
	weapon_sub[playerId] = SUB_TYPES.KATAR
	action[playerId] = ACTION.CREATE
	npcHandler:say("Do you want to spend your dream matter with " .. Config.Create.Clusters .. " clusters of solace and give it a shot? {Yes} or {no}", npc, creature)
	npcHandler:setTopic(playerId, 3)

elseif MsgContains(message, "umbral master katar") and npcHandler:getTopic(playerId) == 2 then
	weapon_sub[playerId] = SUB_TYPES.KATAR
	action[playerId] = ACTION.TRANSFORM
	npcHandler:say("Do you want to spend your Umbral Katar with " .. Config.Transform.Clusters .. " clusters of solace and give it a shot? {Yes} or {no}", npc, creature)
	npcHandler:setTopic(playerId, 3)

elseif MsgContains(message, "umbral katar") and npcHandler:getTopic(playerId) == 2 then
	weapon_sub[playerId] = SUB_TYPES.KATAR
	action[playerId] = ACTION.IMPROVE
	npcHandler:say("Do you want to spend your Crude Umbral Katar with " .. Config.Improve.Clusters .. " clusters of solace and give it a shot? {Yes} or {no}", npc, creature)
	npcHandler:setTopic(playerId, 3)

-- TRATAMENTO DE "NO" ESPECÍFICO PARA KATAR
elseif MsgContains(message, "no") and npcHandler:getTopic(playerId) == 3 and weapon[playerId] == TYPES.KATAR then
	npcHandler:setTopic(playerId, 2)
	npcHandler:say("Do you want to create a {Crude Umbral Katar}, improve a {Crude Umbral Katar} into an {Umbral Katar}, or transform an {Umbral Katar} into an {Umbral Master Katar}?", npc, creature)
	...
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 3 then
		...
		if newItemId and player:getItemCount(newItemId) > 0 then
...
keywordHandler:addGreetKeyword({ "hi" }, {
	...
	text = "Ashari Lillithy. Are you here to {create} weapons or receive your dream {outfit} addon?"
})

keywordHandler:addGreetKeyword({ "ashari" }, {
	npcHandler = npcHandler,
	text = "Ashari Lillithy. You honor me by speaking the old tongue."
})
...
keywordHandler:addKeyword({ "outfit" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Which dream addon are you looking for? The {first} requires a dream warden mask, and the {second} requires a dream warden claw."
})

local node1 = keywordHandler:addKeyword({ "first" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
	text = "To receive the first addon, you must give me 1 dream warden mask. Do you have it with you. {Yes} or {no}?"
})
node1:addChildKeyword({ "yes" }, dreamFirst, {})
node1:addChildKeyword({ "no" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
	text = "Alright then. Come back when you have it.",
	
})

local node2 = keywordHandler:addKeyword({ "second" }, StdModule.say, {
	npcHandler = npcHandler,
	onlyFocus = true,
	text = "To receive the second addon, you must give me 1 dream warden claw. Do you have it with you. {Yes} or {no}?"
})
