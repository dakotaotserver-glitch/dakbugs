...
-- local function creatureSayCallback(npc, creature, type, message)
    ...
    -- elseif player:removeMoneyBank(totalBlessPrice) then
        ...
        -- if player:isPremium() then
            -- -- Premium: Recebe TODAS as blessings exceto a PvP (ID 1)
            -- for id, bless in ipairs(Blessings.All) do
                -- if id ~= 1 and not player:hasBlessing(id) then
                    -- player:addBlessing(id, 1)
                    -- table.insert(blessGiven, bless.name)
                -- end
            -- end
            -- npcHandler:say("As a premium account player, you have received all available blessings, |PLAYERNAME|.", npc, creature)
        -- else
            -- -- Free: Recebe apenas as 5 primeiras (IDs 2 a 6)
            -- for id = 2, 6 do
                -- if not player:hasBlessing(id) then
                    -- player:addBlessing(id, 1)
                    -- table.insert(blessGiven, Blessings.All[id].name)
                -- end
            -- end
            -- npcHandler:say("As a free account player, you have received the 5 standard blessings, |PLAYERNAME|.", npc, creature)
        -- end
        -- player:getPosition():sendMagicEffect(CONST_ME_HOLYAREA)
        -- -- Mensagem para debug, pode remover depois:
        -- if #blessGiven > 0 then
            -- player:sendTextMessage(MESSAGE_STATUS, "Blessings recebidas: " .. table.concat(blessGiven, ", "))
        -- end
    -- else
...
-- npcHandler:setMessage(MESSAGE_GREET, "Greetings, fellow {mission} |PLAYERNAME|!")
...
local function getMissionStatusText(player)
    local q = Storage.Quest.U8_2.TheInquisitionQuest.Questline
    local v = player:getStorageValue(q)
    if v < 1 then
        return "You can join the Inquisition."
    elseif v == 1 then
        return "Take the inquisitor's field guide and start your first mission."
    elseif v == 2 or v == 3 then
        return "Investigate the loyalty of the Thaian guards."
    elseif v == 4 or v == 5 then
        return "Destroy the witches' coven in Femor Hills and bring back their grimoire."
    elseif v == 6 or (v > 6 and v < 11) then
        return "Help the vampire hunter Storkus outside Kazordoon."
    elseif v == 11 or v == 12 or v == 13 then
        return "Exorcise the haunted house in Liberty Bay."
    elseif v == 14 or v == 15 then
        return "Gather 20 demonic essences."
    elseif v == 16 or v == 17 or v == 18 then
        return "Kill the demon Ungreez in Edron."
    elseif v == 19 or v == 20 or v == 21 or v == 22 then
        return "Destroy the shadow nexus in the Demon Forge."
    elseif v >= 23 then
        return "You have completed all missions! Ask me about your blessings or demon hunter outfit."
    else
        return "No current mission."
    end
end

local function getMissionObjective(player)
    local q = Storage.Quest.U8_2.TheInquisitionQuest.Questline
    local v = player:getStorageValue(q)
    if v < 1 then
        return "You can join the Inquisition."
    elseif v == 1 then
        return "Take the inquisitor's field guide from the box in the back room and interrogate the Thaian guards. When finished, report your mission."
    elseif v == 2 or v == 3 then
        return "Investigate the loyalty of the city guards: Walter, Kulag, Grof, Miles, and Tim. When you've spoken with all of them, report to me."
    elseif v == 4 or v == 5 then
        return "Travel to Femor Hills using the flying carpet with codeword 'eclipse'. Destroy the witches' brew with holy water, steal their grimoire, and return it to me."
    elseif v == 6 or (v > 6 and v < 11) then
        return "Speak with Storkus, the vampire hunter, in his cave outside Kazordoon and assist him with his tasks."
    elseif v == 11 or v == 12 or v == 13 then
        return "Exorcise the haunted ruin in Liberty Bay using holy water and return to me when it's done."
    elseif v == 14 or v == 15 then
        return "Slay demons and bring me 20 demonic essences as proof of your deeds."
    elseif v == 16 or v == 17 or v == 18 then
        return "Find and kill the demon Ungreez in the depths of Edron."
    elseif v == 19 or v == 20 or v == 21 or v == 22 then
        return "Destroy the shadow nexus using holy water and defeat all demon lords in the Demon Forge."
    elseif v >= 23 then
        return "You have completed all missions! Ask me about your blessings or demon hunter outfit."
    else
        return "No current mission."
...
local function canReport(player)
    local q = Storage.Quest.U8_2.TheInquisitionQuest.Questline
    local v = player:getStorageValue(q)
    if v == 2 then -- Após interrogar guardas
        local done =
            player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.WalterGuard) == 1 and
            player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.KulagGuard) == 1 and
            player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.GrofGuard) == 1 and
            player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.MilesGuard) == 1 and
            player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.TimGuard) == 1
        return done, 3
    elseif v == 5 then -- Grimoire witches
        return player:getItemCount(7874) >= 1, 6
    elseif v == 10 then -- Missão Storkus
        return true, 11
    elseif v == 13 then -- Haunted Ruin
        return true, 14
    elseif v == 15 then -- Demon Essences
        return player:getItemCount(6499) >= 20, 16
    elseif v == 18 then -- Ungreez
        return true, 20
    elseif v == 22 then -- Shadow Nexus
        return true, 23
    end
    return false, nil
end
...
local function creatureSayCallback(npc, creature, type, message)
    ...
    if MsgContains(msg, "hi") or MsgContains(msg, "hello") then
        local missionText = getMissionStatusText(player)
        npcHandler:say("Greetings, fellow. " .. missionText .. " (You may ask me about {mission}, {report}, {bless}, or {trade}), |PLAYERNAME|!", npc, creature)
        npcHandler:setTopic(playerId, 0)
        return true
    end

    if MsgContains(msg, "trade") then
        npcHandler:say("Let's see what I can offer you.", npc, creature)
        npcHandler:openShopWindow(player, npcConfig.shop)
        npcHandler:setTopic(playerId, 0)
        return true
    end
    ...
    if MsgContains(msg, "mission") then
        ...
        local v = player:getStorageValue(q)
        if v < 1 or v >= 23 then
            npcHandler:say("I have no mission for you at this moment.", npc, creature)
        else
            local missionInfo = getMissionObjective(player)
            npcHandler:say(missionInfo, npc, creature)
        end
        npcHandler:setTopic(playerId, 0)
        return true
    end
    ...
    if MsgContains(msg, "report") then
        local done, nextValue = canReport(player)
        local q = Storage.Quest.U8_2.TheInquisitionQuest.Questline
        local v = player:getStorageValue(q)
        if nextValue then
            if done then
                if v == 2 then
                    npcHandler:say("Indeed, this is exactly what my other sources told me. You can ask me for another mission.", npc, creature)
                    player:setStorageValue(q, 3)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission01, 7)
                elseif v == 5 then
                    npcHandler:say("Fine, fine. You have proven that you can work efficiently. Only further missions will show if you are truly capable. Ask me for another mission.", npc, creature)
                    player:setStorageValue(q, 6)
                    player:removeItem(7874, 1)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission02, 3)
                elseif v == 10 then
                    npcHandler:say("Good, you've returned. Your skill in practical matters seems to be useful. If you're ready for a further mission, just ask.", npc, creature)
                    player:setStorageValue(q, 11)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission03, 6)
                elseif v == 13 then
                    npcHandler:say("Well, this was an easy task, but your next mission will be much more challenging.", npc, creature)
                    player:setStorageValue(q, 14)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission04, 3)
                elseif v == 15 then
                    npcHandler:say("You're indeed a dedicated protector of the true believers. I also have a reward for your efforts. Talk to me about your {demon hunter outfit}. Afterwards, let's talk about the next mission that's awaiting you.", npc, creature)
                    player:setStorageValue(q, 16)
                    player:removeItem(6499, 20)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission05, 2)
                elseif v == 18 then
                    npcHandler:say("So the beast is finally dead! Thank the gods. Our other operatives were not that lucky, though. You will learn more about that in your next {mission}.", npc, creature)
                    player:setStorageValue(q, 20)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission06, 3)
                elseif v == 22 then
                    npcHandler:say("Incredible! You're a true defender of faith! I grant you the title of a High Inquisitor. Ask me about your blessings or outfit.", npc, creature)
                    player:setStorageValue(q, 23)
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission07, 3)
                    player:addAchievement("High Inquisitor")
                    player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.RewardDoor, 1) -- libera a reward door!
                end
            else
                if v == 2 then
                    npcHandler:say("You haven't spoken with all the guards yet.", npc, creature)
                elseif v == 5 then
                    npcHandler:say("You need to bring me the witches' grimoire.", npc, creature)
                elseif v == 15 then
                    npcHandler:say("You need to bring me 20 demonic essences.", npc, creature)
                else
                    npcHandler:say("You haven't completed your mission yet.", npc, creature)
                end
            end
        else
            npcHandler:say("You have nothing to report at this moment.", npc, creature)
        end
        npcHandler:setTopic(playerId, 0)
        return true
    end
    ...
    if MsgContains(msg, "blessing") or MsgContains(msg, "bless") then
        if player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline) == 25 then
            npcHandler:say("Do you want to receive the blessing of the inquisition - which means " .. (missing == 5 and "all five available" or missing) .. " blessings - for " .. totalBlessPrice .. " gold?", npc, creature)
            npcHandler:setTopic(playerId, 7)
        else
            npcHandler:say("You cannot get this blessing unless you have completed The Inquisition Quest.", npc, creature)
    ...
    if MsgContains(msg, "flask") or MsgContains(msg, "special flask") then
    ...
    if MsgContains(msg, "outfit") then
        if player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline) == 16 then
            npcHandler:say("Here is your demon hunter outfit. You deserve it. Unlock more addons by completing more missions.", npc, creature)
            player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline, 17)
            player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission05, 3)
            player:addOutfit(288, 0)
            player:addOutfit(289, 0)
            player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
        elseif player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline) >= 19 and player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline) <= 22 then
            npcHandler:say("Here is your demon hunter outfit addon. You deserve it. Unlock more addons by completing more missions!", npc, creature)
            player:addOutfitAddon(288, 1)
            player:addOutfitAddon(289, 1)
            player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
        elseif player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline) == 23 then
            npcHandler:say("Here is the final addon for your demon hunter outfit. Congratulations!", npc, creature)
            player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline, 24)
            player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Mission07, 4)
            player:setStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.RewardDoor, 1)
            player:addOutfitAddon(288, 2)
            player:addOutfitAddon(289, 2)
            player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
    ...
    -- Falas extras, sem alteração, pode adicionar quantos keywords quiser
    if MsgContains(msg, "dark") then
        npcHandler:say({
            "The dark powers are always present. If a human shows only the slightest weakness, they try to corrupt him and to lure him into their service. ...",
            "We must be constantly aware of evil that comes in many disguises.",
        }, npc, creature)
    ...
    -- Adicione outras falas tradicionais conforme seu script original...
...
keywordHandler:addKeyword({ "church" }, StdModule.say, { npcHandler = npcHandler, text = "The churches of the gods united to fight heresy and dark magic. They are the shield of the true believers, while the inquisition is the sword that fights all enemies of virtuousness." })
keywordHandler:addKeyword({ "inquisitor" }, StdModule.say, { npcHandler = npcHandler, text = "The churches of the gods entrusted me with the enormous and responsible task to lead the inquisition. I leave the field work to inquisitors who I recruit from fitting people that cross my way." })
keywordHandler:addKeyword({ "believer" }, StdModule.say, { npcHandler = npcHandler, text = "Believe on the gods and they will show you the path." })
keywordHandler:addKeyword({ "job" }, StdModule.say, { npcHandler = npcHandler, text = "By edict of the churches I'm the Lord Inquisitor." })
keywordHandler:addKeyword({ "name" }, StdModule.say, { npcHandler = npcHandler, text = "I'm Henricus, the Lord Inquisitor." })

npcHandler:setMessage(MESSAGE_GREET, "Greetings, fellow |PLAYERNAME|! Ask me about {mission}, {report}, {bless} or {trade}.")
npcHandler:setMessage(MESSAGE_FAREWELL, "Always be on guard, |PLAYERNAME|!")
npcHandler:setMessage(MESSAGE_WALKAWAY, "This ungraceful haste is most suspicious!")
