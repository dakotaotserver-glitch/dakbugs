...
addTravelKeyword("gunther", 40, Position(33919, 32010, 6), function(player)
    local podzillaStorage = player:getStorageValue(Storage.U13_40.podzilla.acessopodzilla)

    -- Definindo a posição do teleporte com base no valor da storage
    local destination
    if podzillaStorage < 0 then
        destination = Position(33175, 31765, 6) -- Caso a storage seja menor que 0
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Talk to Gunther.") -- Mensagem em inglês
    else
        destination = Position(33919, 32010, 6) -- Caso a storage seja 0 ou maior
    end

    -- Realiza o teleporte para a posição correspondente
...
--addTravelKeyword("gunther", 20000, Position(33919, 32010, 6))
