...
local function creatureSayCallback(npc, creature, type, message)
	...
	if message:lower() == "vampire" then
    local dusts = {8717, 8718, 8719, 8720}
    local dustNames = {
        [8717] = "Boreth's dust",
        [8718] = "Lersatio's dust",
        [8719] = "Marziel's dust",
        [8720] = "Arthei's dust"
    }
    local missing = {}
    for _, id in ipairs(dusts) do
        if player:getItemCount(id) < 1 then
            table.insert(missing, dustNames[id])
        end
    end

    local addonStorage = 65413
    if #missing == 0 then
        if player:getStorageValue(addonStorage) ~= 1 then
            ...
            -- Dá os addons e o item especial
            ...
            npcHandler:say("Congratulations! You brought all the dusts from the lords of darkness. Here is your Yalaharian outfit addon and a special reward!", npc, creature)
        else
            npcHandler:say("You already received your Yalaharian addon and reward.", npc, creature)
        end
    else
        npcHandler:say("You still need to bring me: " .. table.concat(missing, ", ") .. " to earn the Yalaharian addon and reward.", npc, creature)
    end
    return true
end
