...
-- Mapeamento de armas por vocação base
local config = {
    [VOCATION.BASE_ID.SORCERER] = "wand",
    [VOCATION.BASE_ID.DRUID] = "rod",
    [VOCATION.BASE_ID.PALADIN] = "bow",
    [VOCATION.BASE_ID.KNIGHT] = "sword",
    [VOCATION.BASE_ID.MONK] = "staff", -- Monk adicionado!
}

-- Função robusta para comparar vocação pelo nome, aceitando variações
local function isVocationName(player, message)
    local vocName = player:getVocation():getName():lower()
    local msg = message:lower()
    if msg == vocName then
        return true
    end
    -- Caso Monk, aceitar também "monge" (português)
    if vocName == "monk" and (msg == "monk" or msg == "monge") then
        return true
    end
    return false
end
...
local function greetCallback(npc, creature)
    ...
    npcHandler:setMessage(MESSAGE_GREET,
        "Hello " .. (Player(creature):getSex() == PLAYERSEX_FEMALE and "beautiful lady" or "handsome gentleman") ..
...
local function creatureSayCallback(npc, creature, type, message)
    ...
    --[[ DEBUG opcional para ver o nome da vocação
    print("VOC NAME:", vocation:getName(), "MSG:", message)
    --]]
    ...
    elseif isVocationName(player, message) then
        if npcHandler:getTopic(playerId) == 2 then
            -- Protege caso Monk não esteja no config
            local arma = config[vocationBaseId] or "fists"
            npcHandler:say("Huhu, please don't hurt me with your " .. arma .. "! Reason of your visit?", npc, creature)
