SoulWarQuest = {
	...
	goshnarsCrueltyDefenseChange = 5, -- Defense change, the amount that will decrease or increase defense, the defense cannot decrease more than the monster's original defense amount
	...
	levers = {
		...
		goshnarsGreed = {
			boss = {
				...
				position = Position(33739, 31663, 14),
			},
			requiredLevel = 250,
			playerPositions = {
		...
		goshnarsHatred = {
			...
			monsters = {
				...
				{ name = "symbol of hatred", pos = { x = 33748, y = 31599, z = 14 } },
				{ name = "symbol of hatred", pos = { x = 33738, y = 31599, z = 14 } },
				{ name = "Hateful Soul", pos = { x = 33740, y = 31602, z = 14 } },
				{ name = "Hateful Soul", pos = { x = 33747, y = 31597, z = 14 } },
			},
			requiredLevel = 250,
			playerPositions = {
		...
		goshnarsCruelty = {
			...
			monsters = {
				...
				{ name = "poor soul", pos = { x = 33851, y = 31867, z = 7 } },
			},
			requiredLevel = 250,
			playerPositions = {
		...
		goshnarsMegalomania = {
			...
			monsters = {
				...
				{ name = "Lesser Splinter of Madness", pos = { x = 33710, y = 31635, z = 14 } },
				{ name = "Necromantic Focus", pos = { x = 33717, y = 31634, z = 14 } },
				{ name = "Necromantic Focus", pos = { x = 33703, y = 31634, z = 14 } },
				{ name = "dreadful harvester", pos = { x = 33705, y = 31639, z = 14 } },
				{ name = "dreadful harvester", pos = { x = 33714, y = 31629, z = 14 } },
			},
			requiredLevel = 250,
			playerPositions = {
...
changeMegalomaniaBlue = function()
    ...
    for _, creature in ipairs(zone:getCreatures()) do
        if creature:isMonster() and creature:getName() == "Goshnar's Megalomania" then
            boss = creature
            break
        end
    end
    ...
    -- SALVE UMA REFERÊNCIA DIRETA
    local bossRef = boss:getId() -- ID único, não objeto direto (para não invalidar)

    local function changeBack()
        local bossBack = Creature(bossRef)
        if bossBack and bossBack:isMonster() then
            bossBack:setType("Goshnar's Megalomania Purple")
        end
    end

    SoulWarQuest.changePurpleEvent = addEvent(changeBack, 7000)
end,
...
-- Função para verificar se o ID do piso está na lista de tileItemIds
local function isTileInItemIds(tileId)
	for i = 1, #tileItemIds do
		if tileId == tileItemIds[i] then
			return true
		end
	end
	return false
...
local function revertTilesAndApplyDamage(zonePositions)
	for _, pos in ipairs(zonePositions) do
		...
		if tile and tile:getGround() then
			-- Verificar se o terreno não tem o ID 409 e aplicar o dano
			...
			-- Verificar se algum item na lista tileItemIds está presente no tile
			for i = 1, #tileItemIds do
			...
			-- Se o terreno for ID 410 ou estiver na lista de tileItemIds, e não houver itens na parte superior, enviar o efeito
			local groundId = tile:getGround():getId()
			if (groundId == 410 or isTileInItemIds(groundId)) then
				pos:sendMagicEffect(CONST_ME_REDSMOKE)  -- Enviar o efeito de REDSMOKE na posição
			end
		end
	end
...
function Monster:increaseHatredDamageMultiplier(multiplierCount)
    ...
    -- Adiciona o dano a todos os jogadores dentro da área especificada
    local fromPosition = Position(33699, 31624, 14)
    local toPosition = Position(33722, 31643, 14)

    local spectators = Game.getSpectators(fromPosition, false, true, toPosition.x - fromPosition.x, toPosition.y - fromPosition.y, 0)
    for _, player in pairs(spectators) do
        if player:isPlayer() then
            player:addHealth(-2000) -- Aplica 2000 de dano de morte
            player:say("You are struck by a wave of death energy from Goshnar's Megalomania!", TALKTYPE_MONSTER_SAY)
        end
    end

   -- print("Goshnar's Megalomania has increased its damage multiplier to " .. (attackMultiplier + multiplierCount) .. " and dealt 2000 death damage to players in the area.")
end
    ...
    if tile then
        ...
        if item then
            ...
            if boss then
                ...
                item:remove()
                logger.trace("Necromantic remains strength increased and 2000 death damage applied to nearby players.")
            end
        end
    end
end
	...
	for i = 1, #spectators do
		...
		if tormentCounter <= maxLimit then
			...
			if tormentCounter > 0 then
				local damage = tormentCounter * 55
