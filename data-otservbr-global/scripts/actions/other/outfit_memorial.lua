...
local function updateOutfitCache(storageKey, cache, lastUpdated)
    if not storageKey then
        print("[ERROR] updateOutfitCache: storageKey está nil!")
    ...
    if resultId then
        repeat
            ...
            if newCache[value] then
                table.insert(newCache[value], Result.getString(resultId, "name"))
            end
...
function outfitMemorial.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	royalOutfitCache, lastUpdatedRoyal = updateOutfitCache(Storage.Quest.U12_80.RoyalCostumeOutfits, royalOutfitCache, lastUpdatedRoyal)
