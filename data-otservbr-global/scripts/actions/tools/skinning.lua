...
-- ========================
-- Verificações especiais: stake (5942) em 8738 nas áreas dos bosses
-- ========================
local specialDustAreas = {
    -- [centerPos] = { storage, item, mensagem }
    [{x=32939, y=31478, z=1}] = { storage = 65139, item = 8717, message = "You got some ounces of Boreth's dust. This should be enough proof that you killed him." },
    [{x=32967, y=31459, z=1}] = { storage = 65140, item = 8718, message = "You got some ounces of Lersatio's dust. This should be enough proof that you killed him." },
    [{x=32940, y=31460, z=1}] = { storage = 65141, item = 8719, message = "You got some ounces of Marziel's dust. This should be enough proof that you killed him." },
    [{x=32953, y=31442, z=1}] = { storage = 65142, item = 8720, message = "You got some ounces of Arthei's dust. This should be enough proof that you killed him." },
}

if item.itemid == 5942 and target.itemid == 8738 then
    local pos = toPosition or target:getPosition()
    for center, conf in pairs(specialDustAreas) do
        local cx, cy, cz = center.x, center.y, center.z
        local areaFrom = Position(cx - 3, cy - 3, cz)
        local areaTo   = Position(cx + 3, cy + 3, cz)
        if pos.x >= areaFrom.x and pos.x <= areaTo.x and
           pos.y >= areaFrom.y and pos.y <= areaTo.y and
           pos.z == areaFrom.z then
            if player:getStorageValue(conf.storage) < 1 then
                target:transform(8742)
                player:addItem(conf.item, 1)
                player:sendTextMessage(MESSAGE_EVENT_ADVANCE, conf.message)
                player:setStorageValue(conf.storage, 1)
            else
                player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have already taken this dust.")
            end
            return true
        end
    end
