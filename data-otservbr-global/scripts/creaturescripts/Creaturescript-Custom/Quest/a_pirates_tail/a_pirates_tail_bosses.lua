...
function tentuglysHeadDeath.onDeath(creature, corpse, killer, mostDamage, unjustified, mostDamage_unjustified)
	...
	for key, value in pairs(damageMap) do
		...
		if player and player:getStorageValue(Storage.Quest.U12_60.APiratesTail.TentuglyKilled) < 2 then
			player:setStorageValue(Storage.Quest.U12_60.APiratesTail.TentuglyKilled, 2) -- Access to wreckoning
			player:addMount(175)
			player:getPosition():sendMagicEffect(171)
			player:addItem(35508, 1)
		end
	end
end
