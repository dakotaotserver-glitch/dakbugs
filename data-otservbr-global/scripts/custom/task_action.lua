function endTaskModalWindow(player, storage)
	-- Definindo uma variável local para armazenar o storage para contagem
	local storageCounter = storage -- Usando o próprio storage como contador
		...
		if player:getStorageValue(taskOptions.bonusReward) >= 1 then
			...
			for _, info in pairs(data.rewards) do
				...
				elseif tonumber(info[1]) then
					...
					-- Substituindo storagecheck por storageCounter
					player:setStorageValue(storageCounter, player:getStorageValue(storageCounter) + 1)
				end
			end
		else
			if taskOptions.selectLanguage == 1 then
...
function task.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local function getPositionTileItem(pos)
		...
		return math.abs(xDiff) <= 1 and math.abs(yDiff) <= 1
	end
