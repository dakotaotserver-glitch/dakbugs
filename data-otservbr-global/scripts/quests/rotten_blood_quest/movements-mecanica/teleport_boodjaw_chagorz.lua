local function isChagorzInArea()
    local fromPosition = Position(33032, 32356, 15)
    local toPosition = Position(33054, 32376, 15)

    local spectators = Game.getSpectators(fromPosition, false, false, 22, 22, 20, 20)
    for _, spectator in pairs(spectators) do
        if spectator:isMonster() and spectator:getName() == "Chagorz" then
            return true
        end
    end
    return false
...
local function damagePlayersInAreaIfElderBloodjawPresent()
    ...
    if countElderBloodjawInArea() == 2 then
        local players = Game.getSpectators(fromPosition, false, true, 22, 22, 20, 20) -- Obter apenas jogadores
...
local function createItemLoop()
    if isChagorzInArea() then
        local itemPosition = Position(33043, 32366, 15)
        local item = Game.createItem(44049, 1, itemPosition)
        if item then
            item:setActionId(33119)
        end
