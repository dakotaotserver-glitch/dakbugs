...
local playerPosition = Position(32958, 32309, 8) -- Posição para onde o jogador é teleportado
...
-- Configuração para modo de teste (desabilita o cooldown)
local testMode = false -- Altere para false para desativar o modo de teste
...
-- Função para formatar o tempo restante em horas e minutos
...
-- Função resetRoom corrigida para não considerar o jogador que está tentando entrar
local function resetRoom(position, name, playerId)
    -- Definindo os limites exatos da sala baseado nas coordenadas fornecidas
    local roomFromPos = Position(32944, 32304, 8)  -- Canto inferior esquerdo
    local roomToPos = Position(32959, 32314, 8)    -- Canto superior direito
    
    -- Verifica se há jogadores na sala usando as coordenadas exatas
    local spectators = Game.getSpectators(position, false, true, 
        (roomToPos.x - position.x), (position.x - roomFromPos.x),
        (roomToPos.y - position.y), (position.y - roomFromPos.y))
    
    for _, spectator in pairs(spectators) do
        -- Ignora o jogador que está tentando entrar na sala
        if spectator:isPlayer() and spectator:getId() ~= playerId then
            return false
        end
    end
    
    -- Remove qualquer monstro com o nome especificado na sala (case-insensitive)
    local monsters = Game.getSpectators(position, false, false, 
        (roomToPos.x - position.x), (position.x - roomFromPos.x),
        (roomToPos.y - position.y), (position.y - roomFromPos.y))
	...
	if player then
		if item.actionid == 4930 then
			if position == lastTeleport then
				-- Passa o ID do jogador para a função resetRoom para ignorá-lo na verificação
				if resetRoom(scorpionPosition, bossName, player:getId()) then
					-- Verifica se está em modo de teste ou se o cooldown já passou
					if testMode or player:getStorageValue(scorpionTimer) < os.time() then
						startBattle(player:getId(), playerPosition, bossName, scorpionPosition)
						...
						-- Só define o cooldown se não estiver em modo de teste
						...
						local timeLeft = player:getStorageValue(scorpionTimer) - os.time()
						local formattedTime = formatTimeRemaining(timeLeft)
						player:sendCancelMessage(string.format("Voce precisa esperar %s antes de desafiar o Furious Scorpion novamente.", formattedTime))
