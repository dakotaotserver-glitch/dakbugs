...
local function transformCrystal(playerId)
    ...
    for c = 1, #crystals do
        ...
        if crystal.globalStorage then -- proteção extra
            player:setStorageValue(crystal.globalStorage, 0)
        end
        if Storage.Quest.U10_90.FerumbrasAscension.Crystals.AllCrystals then
...
function ascendantBossesKill.onDeath(creature)
    ...
    if creature:getName():lower() == "ferumbras mortal shell" then
        onDeathForDamagingPlayers(creature, function(creature, player)
            addEvent(transformCrystal, 2 * 60 * 1000, player:getId())
        end)
    end
