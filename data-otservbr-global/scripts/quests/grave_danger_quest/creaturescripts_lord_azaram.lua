local config = {
	...
	bossPos = Position(33482, 31477, 13),
	tainted = {
...
function azaram_health.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType)
	...
	local health = creature:getMaxHealth() * 0.20
	local damageStorage = creature:getStorageValue(1)
	if damageStorage == -1 then damageStorage = 0 end

	local phaseCount = creature:getStorageValue(1000)
	if phaseCount == -1 then phaseCount = 0 end

	local realDamage = math.abs(primaryDamage) + math.abs(secondaryDamage)
	local newDamage = damageStorage + realDamage

	if phaseCount >= 4 then
		return primaryDamage, primaryType, -secondaryDamage, secondaryType
	end

	if newDamage >= health then
		creature:setStorageValue(1, 0)
		creature:setStorageValue(1000, phaseCount + 1)
		...
		local soul = Game.createMonster("Azaram's Soul", config.centerRoom, false, true)

		for _, pos in ipairs(config.tainted) do
...
function soul_heal.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType)
	...
	if primaryType == COMBAT_HEALING then
		...
		if healStorage == -1 then healStorage = 0 end

		if healthPercent < 100 and healthPercent >= healStorage * 15 then
