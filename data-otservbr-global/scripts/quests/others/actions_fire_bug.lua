...
function othersFireBug.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    -- NOVA MECÂNICA: Encantamento temporário do Fire Bug (actionid 33169)
    if target.actionid == 33169 then
        item:setActionId(33171)
        toPosition:sendMagicEffect(CONST_ME_MAGIC_RED)
        addEvent(function()
            if player:getItemCount(5467) > 0 then
                local fireBug = player:getItemById(5467)
                if fireBug and fireBug:getActionId() == 33171 then
                    fireBug:setActionId(0)
                end
            end
        end, 60 * 1000) -- 1 minuto
        return true
    end
