...
function teleportPlayersAfterDelay(upCorner, downCorner, delay, position)
    addEvent(function()
            ...
            for j = upCorner.y, downCorner.y do
                for k = upCorner.z, downCorner.z do
                    ...
                    if tile then
                        local creatures = tile:getCreatures()
                        if creatures and #creatures > 0 then
                            for _, cid in pairs(creatures) do
                                local player = Player(cid)
                                if player then
                                    player:teleportTo(position)
                                    position:sendMagicEffect(CONST_ME_TELEPORT)
                                    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You stayed too long and were teleported out!")
                                end
                            end
                        end
                    end
...
function clearDevourer()
    local upConer = { x = 32260, y = 31336, z = 14 }
    local downConer = { x = 32283, y = 31360, z = 14 }
