...
function creaturescripts_dreamCourtsDeath.onDeath(creature, corpse, killer, mostDamageKiller, lastHitUnjustified)
	...
	-- Se morrer Plant Abomination, recria Plant Attendant
	if cName == "plant abomination" then
		Game.createMonster("Plant Attendant", cPos)
	end

	-- Se morrer Plant Attendant, recria outro igual
	if cName == "plant attendant" then
		Game.createMonster("Plant Attendant", cPos)
	end

	-- Questlog boss tracking
	for _, k in pairs(questlog) do
		if cName == k.bossName:lower() then
	...
	-- Contador de summons de Alptramun
