...
function pitsOfInfernoBosses.onThink(interval, lastExecution)
	...
	if not monster then
		logger.error("[PitsOfInfernoBosses] - Failed to spawn %s", spawn.monster)
		return true
	end

	Game.broadcastMessage(string.format("O boss %s apareceu nas profundezas do Pits of Inferno!", spawn.monster), MESSAGE_EVENT_ADVANCE)
	return true
end

pitsOfInfernoBosses:interval(86400000) -- 24horas
