...
bool IOBestiary::parseOffensiveCharmCombat(const std::shared_ptr<Charm> &charm, const std::shared_ptr<Player> &player, const std::shared_ptr<Creature> &target, CombatDamage &charmDamage, CombatParams &charmParams) {
	int32_t value = 0;
	std::shared_ptr<ConditionSpeed> cripple = nullptr;
	...
	switch (charm->id) {
		...
		case CHARM_MAJOR_CARNAGE:
			...
			for (int8_t x = -1; x <= 1; x++) {
				for (int8_t y = -1; y <= 1; y++) {
					if ((x != 0 && y == 0) || (x == 0 && y != 0)) { // Cross = [[ 0, 1, 0 ], [1, 0, 1], [ 0, 1, 0]]
						Position damagePosition = Position(targetPosition.x + x, targetPosition.y + y, targetPosition.z);
						...
						if (topCreature && topCreature->getType() == CREATURETYPE_MONSTER) {
							parseCharmCarnage(charm, player, topCreature, (-value * charm->percent) / 100);
						}
					}
				}
			}
		...
		case CHARM_MINOR_CRIPPLE:
			cripple = std::static_pointer_cast<ConditionSpeed>(Condition::createCondition(CONDITIONID_COMBAT, CONDITION_PARALYZE, 10000, 0));
	...
	parseOffensiveCharmCombatDamage(charm, (-value * charm->percent) / 100, charmDamage, charmParams);
...
std::map<uint16_t, std::string> IOBestiary::findRaceByName(const std::string &race, bool Onlystring /*= true*/, BestiaryType_t raceNumber /*= BESTY_RACE_NONE*/) const {
	...
	if (Onlystring) {
		for (const auto &[monsterTypeRaceId, monsterTypeName] : bestiaryListMap) {
			const auto tmpType = g_monsters().getMonsterType(monsterTypeName);
			if (tmpType && tmpType->info.bestiaryClass == race) {
				race_list.insert({ monsterTypeRaceId, monsterTypeName });
			}
		}
	} else {
		for (const auto &[monsterTypeRaceId, monsterTypeName] : bestiaryListMap) {
...
void IOBestiary::resetCharmRuneCreature(const std::shared_ptr<Player> &player, const std::shared_ptr<Charm> &charm) const {
	...
	int32_t value = bitToggle(player->getUsedCharmRunesBit(), charm, false);
	player->setUsedCharmRunesBit(value);
...
bool IOBestiary::setCharmRuneCreature(const std::shared_ptr<Player> &player, const std::shared_ptr<Charm> &charm, uint16_t raceid) const {
	if (!player || !charm) {
		return false;
	}
	...
	for (const auto &charm : charmList) {
		...
		const auto raceIdByCharm = player->getRaceIdByCharmsArray(charm->id);
		if (raceIdByCharm == raceid) {
			count++;
		}
	}
	...
	int32_t value = bitToggle(player->getUsedCharmRunesBit(), charm, true);
	player->setUsedCharmRunesBit(value);
...
std::list<charmRune_t> IOBestiary::getCharmUsedRuneBitAll(const std::shared_ptr<Player> &player) {
	int32_t input = player->getUsedCharmRunesBit();
...
uint16_t IOBestiary::getBestiaryRaceUnlocked(const std::shared_ptr<Player> &player, BestiaryType_t race) const {
	...
	for (const auto &[_, monsterTypeName] : bestiaryListMap) {
...
void IOBestiary::sendBuyCharmRune(const std::shared_ptr<Player> &player, uint8_t action, charmRune_t charmId, uint16_t raceId) {
	...
	if (action == 0) {
		...
		int32_t value = bitToggle(player->getUnlockedCharmRunesBit(), charm, true);
		player->setUnlockedCharmRunesBit(value);
	} else if (action == 1) {
		std::list<charmRune_t> usedRunes = getCharmUsedRuneBitAll(player);
		...
		if (mType) {
			if (charm->category == CHARM_MAJOR) {
				...
				if (killedAmount < mType->info.bestiaryToUnlock) {
					player->sendFYIBox("You need to kill more monsters to unlock this charm.");
					return;
				}
			}

			const auto &[activeMajorCharm, activeMinorCharm] = getCharmFromTarget(player, mType, charm->category);
			if ((charm->category == CHARM_MAJOR && activeMajorCharm != CHARM_NONE) || (charm->category == CHARM_MINOR && activeMinorCharm != CHARM_NONE)) {
				player->sendFYIBox(fmt::format("You already have this monster set on another {} Charm!", charm->category == CHARM_MAJOR ? "Major" : "Minor"));
				return;
			}

			if (setCharmRuneCreature(player, charm, raceId)) {
				if (!player->isPremium()) {
		...
		player->setUsedCharmRunesBit(0);
		player->setUnlockedCharmRunesBit(0);
...
std::vector<uint16_t> IOBestiary::getBestiaryFinished(const std::shared_ptr<Player> &player) const {
	...
	finishedMonsters.reserve(bestiaryListMap.size());
