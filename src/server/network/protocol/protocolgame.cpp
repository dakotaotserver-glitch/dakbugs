				...
				for (uint8_t i = 0; i < slots; i++) {
					...
					if (imbuementInfo.duration > 0) {
						...
						if (imbuement.combatType != COMBAT_NONE) {
							msg.addDouble(getCipbiaElement(imbuement.combatType));
...
void ProtocolGame::parsePacketFromDispatcher(NetworkMessage &msg, uint8_t recvbyte) {
	...
	switch (recvbyte) {
		...
		case 0xB1:
			...
			break;
		case 0xB2:
			parseImbuementWindow(msg); // Summer Update 2025
			break;
		case 0xB3:
			parseWeaponProficiency(msg); // Summer Update 2025
			break;
		case 0xBA:
...
void ProtocolGame::parseBestiarysendMonsterData(NetworkMessage &msg) {
	...
	auto ait = bestiaryListMap.find(raceId);
	if (ait != bestiaryListMap.end()) {
...
void ProtocolGame::parseCyclopediaMonsterTracker(NetworkMessage &msg) {
	...
	const auto &bestiaryListMap = g_game().getBestiaryList();
	auto it = bestiaryListMap.find(monsterRaceId);
...
void ProtocolGame::parseSendBuyCharmRune(NetworkMessage &msg) {
	...
	const charmRune_t charmId = static_cast<charmRune_t>((action != 3) ? msg.getByte() : 0);
	const uint16_t raceId = (action == 1) ? msg.get<uint16_t>() : 0;
...
void ProtocolGame::sendBestiaryCharms() {
	...
	for (const auto &c_type : charmList) {
		...
		if (g_iobestiary().hasCharmUnlockedRuneBit(c_type, player->getUnlockedCharmRunesBit())) {
	...
	if (player->isPremium() && player->hasCharmExpansion()) {
		...
		availableCharmSlots = totalCharmSlots - static_cast<uint8_t>(usedCharms.size());
	}
	...
	for (charmRune_t charmRune : g_iobestiary().getCharmUsedRuneBitAll(player)) {
		if (const auto &tmpCharm = g_iobestiary().getBestiaryCharm(charmRune)) {
...
void ProtocolGame::sendCyclopediaCharacterMiscStats() {
	...
	msg.addByte(magic_enum::enum_count<Blessings>()); // Mudar
...
void ProtocolGame::sendOutfitWindow() {
	...
	constexpr uint16_t limitOutfits = std::numeric_limits<uint16_t>::max();
	uint16_t outfitSize = 0;
	msg.skipBytes(2);
	...
	auto startMounts = msg.getBufferPosition();
	constexpr uint16_t limitMounts = std::numeric_limits<uint16_t>::max();
	uint16_t mountSize = 0;
	msg.skipBytes(2);
	...
	auto startFamiliars = msg.getBufferPosition();
	constexpr uint16_t limitFamiliars = std::numeric_limits<uint16_t>::max();
...
void ProtocolGame::sendPreyData(const std::unique_ptr<PreySlot> &slot) {
	...
	if (slot->state == PreyDataState_Locked) {
		...
		const auto &bestiaryListMap = g_game().getBestiaryList();
		msg.add<uint16_t>(static_cast<uint16_t>(bestiaryListMap.size()));
		std::for_each(bestiaryListMap.begin(), bestiaryListMap.end(), [&msg](auto mType) {
...
void ProtocolGame::sendTaskHuntingData(const std::unique_ptr<TaskHuntingSlot> &slot) {
	...
	if (slot->state == PreyTaskDataState_Locked) {
		...
		const auto &bestiaryListMap = g_game().getBestiaryList();
		msg.add<uint16_t>(static_cast<uint16_t>(bestiaryListMap.size()));
		std::for_each(bestiaryListMap.begin(), bestiaryListMap.end(), [&msg, user](auto mType) {
			msg.add<uint16_t>(mType.first);
...
void ProtocolGame::syncOpenContainers() {
	...
	if (!openContainers.empty()) {
		for (const auto &[containerId, openContainer] : openContainers) {
			auto opcontainer = openContainer.container;
			bool hasParent = (opcontainer->getParent() != nullptr);
			sendContainer(containerId, opcontainer, hasParent, openContainer.index);
		}
	}
}
...
void ProtocolGame::parseSelectSpellAimProtocol(NetworkMessage &msg) {
	...
	for (auto i = 1; i <= spellListSize; i++) {
		...
		g_logger().warn("[{}] spellListSize {} spellId: {}, spellAim: {}", __FUNCTION__, spellListSize, spellId, spellAim);
