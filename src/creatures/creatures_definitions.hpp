...
enum charmRune_t : int8_t {
	...
	CHARM_LAST = CHARM_MAJOR_OVERFLUX,
};
...
struct CombatDamage {
	...
	bool isEmpty() const {
		return primary.type == COMBAT_NONE && primary.value == 0 &&
			secondary.type == COMBAT_NONE && secondary.value == 0 &&
			origin == ORIGIN_NONE && critical == false && affected == 1 && 
			extension == false && exString.empty() && fatal == false &&
			criticalDamage == 0 && criticalChance == 0 && damageMultiplier == 0 &&
			damageReductionMultiplier == 0 && healingMultiplier == 0 &&
			manaLeech == 0 && manaLeechChance == 0 && lifeLeech == 0 &&
			lifeLeechChance == 0 && healingLink == 0 &&
			instantSpellName.empty() && runeSpellName.empty();
	}
};
