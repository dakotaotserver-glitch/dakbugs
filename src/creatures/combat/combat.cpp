...
static void applyImproveMonkAttackSpender(const std::shared_ptr<Player> &player, CombatDamage &damage) {
	...
	g_logger().warn("[{}] harmony improve damage antes {}", __FUNCTION__, damage.primary.value);
...
#ifdef DEBUG_UPDATE15_LOG
	g_logger().warn("[{}] harmony improve damage depois {} / totalBonusPercent {} / multiplier {}", __FUNCTION__, damage.primary.value, totalBonusPercent, multiplier);
#endif // DEBUG_UPDATE15_LOG
}
	...
	for (const auto &condition : params.conditionList) {
		...
		if (player) {
			...
			if (player->isImmuneCleanse(condition->getType())) {
				...
				if (const auto &cleanseCharm = player->isApplyCharm(CHARM_MINOR_CLEANSE, caster->getName())) {
					...
					if (!cleanseCharm->cancelMessage.empty()) {
						player->sendCancelMessage(cleanseCharm->cancelMessage);
					}
				...
				if (!params.aggressive || (caster != creature && Combat::canDoCombat(caster, creature, params.aggressive) == RETURNVALUE_NOERROR)) {
					...
					if (casterPlayer) {
						casterPlayer->wheel().updateBeamMasteryDamage(tmpDamage, beamAffectedTotal, beamAffectedCurrent);
										...
										/*
					CombatDamage damageTmp;
					const auto &monster = creature->getMonster();
					if (monster && casterPlayer) {
						bool canApplyLowOrSavage = monster->checkCanApplyCharm(casterPlayer, CHARM_MAJOR_LOWBLOW) || monster->checkCanApplyCharm(casterPlayer, CHARM_MAJOR_SAVAGEBLOW);
						if (canApplyLowOrSavage) {
							damageTmp = tmpDamage;
						}
					}
					*/
...
void Combat::applyExtensions(const std::shared_ptr<Creature> &caster, const std::vector<std::shared_ptr<Creature>> &targets, CombatDamage &damage, const CombatParams &params) {
	...
	if (const auto &player = caster->getPlayer()) {	
		...
		// PEGUE O ATTACKEDCREATURE AQUI:
		const auto attackedCreature = player->getAttackedCreature();
