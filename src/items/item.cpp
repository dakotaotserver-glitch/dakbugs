...
uint8_t Item::getTier() const {
	...
	const uint8_t tier = getAttribute<uint8_t>(ItemAttribute_t::TIER);
...
std::string Item::parseClassificationDescription(const std::shared_ptr<Item> &item) {
	...
	if (item && item->getClassification() >= 1) {
		...
		string << '\n' << "Classification: " << int(item->getClassification()) << " Tier: " << int(tier);
		...
		if (tier != 0) {
			...
			if (itemType.weaponType != WEAPON_NONE) {
				...
				const auto category = g_game().getObjectCategory(item);
				double_t chance = 0.f;
				std::string label;
				...
				switch (category) {
					...
					case OBJECTCATEGORY_ARMORS:
						chance = item->getDodgeChance();
						label = "Ruse";
						break;
					case OBJECTCATEGORY_LEGS:
						chance = item->getTranscendenceChance();
						label = "Transcendence";
						break;
					case OBJECTCATEGORY_BOOTS:
						chance = item->getAmplificationChance();
						label = "Amplification";
						break;
					default:
						break;
				...
				if (chance > 0.f && !label.empty()) {
					string << fmt::format(" ({:.2f}% {}).", chance, label);
				}
			}
		}
	}
