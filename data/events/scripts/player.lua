...
-- function Player:onReportRuleViolation(targetName, reportType, reportReason, comment, translation)
	-- logger.info("[DEBUG] ReportRuleViolation ativado por: " .. self:getName())
...
-- Tempo de cooldown entre reports (em segundos)
local REPORT_COOLDOWN = 1

-- Função para verificar se o player pode enviar novo report
local function hasPendingReport(playerGuid, targetName, reportType)
	local player = Player(playerGuid)
	if not player then
		return false
	...
	local name = player:getName():gsub("%s+", "_")
	local cooldownFile = string.format("%s/reports/players/%s/_cooldown.txt", CORE_DIRECTORY, name)
	local now = os.time()
	...
	if file then
		lastTime = tonumber(file:read("*a")) or 0
		file:close()
	end

	if now - lastTime < REPORT_COOLDOWN then
		local secondsLeft = REPORT_COOLDOWN - (now - lastTime)
		player:sendCancelMessage("You must wait " .. secondsLeft .. " seconds before sending another report.")
		return true
	end

	-- Verifica se já reportou esse jogador com esse tipo
	local reportFile = string.format("%s/reports/players/%s-%s-%d.txt", CORE_DIRECTORY, name, targetName, reportType)
	local existing = io.open(reportFile, "r")
	if existing then
		existing:close()
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have already reported this player for this type of violation.")
		return true
	end

	-- Atualiza cooldown
	local cooldownSave = io.open(cooldownFile, "w")
	if cooldownSave then
		cooldownSave:write(now)
		cooldownSave:close()
	end
...
function Player:onReportRuleViolation(targetName, reportType, reportReason, comment, translation)
	...
	local reportDir = string.format("%s/reports/players", CORE_DIRECTORY)
	FS.mkdir_p(reportDir)

	if hasPendingReport(self:getGuid(), targetName, reportType) then
		return
	end

	local filePath = string.format("%s/%s-%s-%d.txt", reportDir, name, targetName, reportType)
	...
	file:write("Reported by: " .. self:getName() .. "\n")
	...
	self:sendTextMessage(
		MESSAGE_EVENT_ADVANCE,
		string.format(
			"Thank you for reporting %s. Your report will be processed by %s team.",
			targetName,
			configManager.getString(configKeys.SERVER_NAME)
		)
	)
	logger.info(string.format("[RULE REPORT] %s reported %s for type %d", name, targetName, reportType))
	return
end
...
function Player:onReportBug(message, position, category)
	local name = self:getName()
	local nameSanitized = name:gsub("%s+", "_")
	local playerPosition = self:getPosition()
	local timestamp = os.date("%Y-%m-%d_%H-%M-%S")
	local playerIp = Game.convertIpToString(self:getIp())

	local folderPath = string.format("%s/reports/bugs/%s", CORE_DIRECTORY, nameSanitized)
	FS.mkdir_p(folderPath)

	local fileName = string.format("%s/%s.txt", folderPath, timestamp)
	...
	file:write("IP: " .. playerIp .. "\n")
	if category == BUG_CATEGORY_MAP and position then
		file:write(string.format("Map Position: %d, %d, %d\n", position.x, position.y, position.z))
	end
	...
	local mapPositionText = ""
	if category == BUG_CATEGORY_MAP and position then
		mapPositionText = string.format("🗺️ **Map Position:** %d, %d, %d\n", position.x, position.y, position.z)
	end
	...
	if type(sendDiscordBugReport) == "function" then
		sendDiscordBugReport(discordMessage)
	end
