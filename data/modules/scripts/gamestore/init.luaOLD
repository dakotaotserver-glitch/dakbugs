...
function parseBuyStoreOffer(playerId, msg)
	...
	-- Função auxiliar segura para verificar se o tipo de oferta é válido
local function isValidOfferType(offerType)
	for _, v in pairs(GameStore.OfferTypes) do
		if v == offerType then
			return true
		end
	end
...
-- Validação principal da oferta
if
	not isValidOfferType(offer.type) -- tipo inválido
	or not player -- jogador não encontrado
	or (player:getVocation():getId() == 0 and not GameStore.haveOfferRook(id)) -- Rookgaard sem permissão
	or not offer -- oferta ausente
	or offer.type == GameStore.OfferTypes.OFFER_TYPE_NONE -- oferta desativada
	or not offer.id -- oferta sem ID
then
...
function Player.canBuyOffer(self, offer)
	...
	-- Apenas validações de lógica (já tem o item, limite de blessing, outfit, etc)
	if offer.disabled or not offer.type then
		disabled = 1
...
-- CÓDIGO CORRIGIDO (AJUSTANDO O LOOP)
function GameStore.processItemPurchase(player, offerId, offerCount, movable, setOwner)
    ...
    for t = 1, offerCount do
        -- A mágica está aqui: trocamos 'offerCount' por '1'
...
function Player.canRemoveCoins(self, coins)
    -- Modificada para considerar o saldo combinado
    return (self:getTibiaCoins() + self:getTransferableCoins()) >= coins
end

function Player.removeCoinsBalance(self, coins)
    -- Verificar saldo total combinado
    if (self:getTibiaCoins() + self:getTransferableCoins()) >= coins then
        -- Calcular quanto remover de cada tipo
        local regularCoins = self:getTibiaCoins()
        local regularCoinsToRemove = math.min(regularCoins, coins)
        local transferableCoinsToRemove = 0
        
        if regularCoinsToRemove < coins then
            transferableCoinsToRemove = coins - regularCoinsToRemove
        end
        
        -- Remover coins regulares
        if regularCoinsToRemove > 0 then
            self:removeTibiaCoins(regularCoinsToRemove)
        end
        
        -- Remover transferable coins
        if transferableCoinsToRemove > 0 then
            self:removeTransferableCoins(transferableCoinsToRemove)
        end
        
        -- Atualizar saldo no cliente
...
function Player.makeCoinTransaction(self, offer, desc)
    -- Calcular preço correto para ExpBoost
    ...
    if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
        ...
        if expBoostCount <= 5 then
            offerPrice = GameStore.ExpBoostValues[expBoostCount]
        end
    end

    -- Verificar saldo total
    local totalCoins = self:getTibiaCoins() + self:getTransferableCoins()
    if totalCoins < offerPrice then
        return false
    end

    -- Preparar descrição
    local transactionDesc = offer.name
    if desc then
        transactionDesc = offer.name .. " (" .. desc .. ")"
    end

    -- Calcular quanto remover de cada tipo
    local regularCoins = self:getTibiaCoins()
    local regularCoinsToRemove = math.min(regularCoins, offerPrice)
    local transferableCoinsToRemove = 0
    
    if regularCoinsToRemove < offerPrice then
        transferableCoinsToRemove = offerPrice - regularCoinsToRemove
    end
    ...
    -- Remover coins regulares
    if regularCoinsToRemove > 0 then
        success = self:removeTibiaCoins(regularCoinsToRemove)
        if not success then
            return false
        end
    end
    ...
    -- Remover transferable coins
    if transferableCoinsToRemove > 0 and success then
        success = self:removeTransferableCoins(transferableCoinsToRemove)
        if not success then
            -- Restaurar coins regulares se falhar
            if regularCoinsToRemove > 0 then
                self:addTibiaCoins(regularCoinsToRemove)
            end
            return false
        end
    end

    -- Atualizar contador de XP Boost se aplicável
    if success and offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
        local playerKV = self:kv()
        local expBoostCount = playerKV:get(GameStore.Kv.expBoostCount) or 0
        if expBoostCount == -1 or expBoostCount == 0 or expBoostCount > 5 then
            expBoostCount = 1
    ...
    if success then
        -- Atualizar o saldo no cliente
        sendStoreBalanceUpdating(self:getId(), true)
        
        -- Determinar tipo principal para histórico
        local historyCoinType = GameStore.CoinType.Transferable
        if regularCoinsToRemove > 0 then
            historyCoinType = GameStore.CoinType.Coin
        end
        
        GameStore.insertHistory(self:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, transactionDesc, offerPrice * -1, historyCoinType)
    end
...
function Player.canPayForOffer(self, coinsToRemove, coinType)
    -- Ignorar coinType e verificar saldo total combinado
    return (self:getTibiaCoins() + self:getTransferableCoins()) >= coinsToRemove
end
