-- Armazenamento e controle de storage dos monstros
...
-- Sistema de HP compartilhado entre monstros
...
function Monster.inSharedLife(self)
	...
	if storage < 1 then return false end
	for id, pid in pairs(hpCompartilhada) do
		if storage == id then
			for _, mid in pairs(pid.monsters) do
...
function updateMonstersSharedLife(hpid, amount, orign, _type, kill)
	if not hpCompartilhada[hpid] then return false end
...
function Monster.onReceivDamageSL(self, damage, tp, killer)
	if not self:inSharedLife() then return true end
	local storage = self:getStorageValue("shared_storage")
	if storage < 1 then return false end
...
-- Ajuste com proteção de slot no loot
local equipmentBags = {
	BAG_YOU_DESIRE,
	PRIMAL_BAG,
	BAG_YOU_COVET,
...
do
	...
	function MonsterType.getBossReward(self, lootFactor, topScore, equipmentOnly, lootTable, player)
		...
		local loot = self:generateLootRoll({
			factor = lootFactor,
			gut = false,
		...
		-- Proteção para Loot Pouch: impedir exceder capacidade
		local pouch = findLootPouch and findLootPouch(player)
		if pouch then
			local container = pouch:getContainer()
			if container then
				for i, item in ipairs(loot) do
					if container:getSize() < container:getCapacity() then
						container:addItem(item.itemid, item.count or 1)
					else
						player:sendTextMessage(MESSAGE_STATUS_WARNING, "Loot pouch cheia. Parte do loot não foi adicionado.")
						break
					end
				end
			end
