...
function playersOnline.onSay(player, words, param)
	...
	for _, targetPlayer in ipairs(players) do
		if hasAccess or not targetPlayer:isInGhostMode() then
			local prisonStorage = targetPlayer:getStorageValue(59998) -- prison.cellStorage
			if prisonStorage and prisonStorage > 0 then
				table.insert(onlineList.Prison, targetPlayer)
			elseif _G.OnExerciseTraining[targetPlayer:getId()] then
	...
	local onlineCount = #onlineList.Training + #onlineList.Idle + #onlineList.Active + #onlineList.Prison
	player:sendTextMessage(MESSAGE_ATTENTION, ("%d players online | Training: %d | Idle: %d | Active: %d | Prison: %d"):format(
		onlineCount, #onlineList.Training, #onlineList.Idle, #onlineList.Active, #onlineList.Prison))
