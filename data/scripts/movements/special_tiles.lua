...
local function checkAndSendDepotMessage(player)
    ...
    local currentTile = currentPos:getTile()
    if currentTile then
        local depotItem = currentTile:getItemByType(ITEM_TYPE_DEPOT)
        if depotItem then
            local depotItems = 0
...
for index, value in pairs(increasing) do tile:id(index) end
tile:type("stepin")
tile:register()

local tileOut = MoveEvent()

function tileOut.onStepOut(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
	...
	local pressedId = item:getId()
	local newId = decreasing[pressedId]
	...
	addEvent(function()
		...
		if tile then
			...
			if ground then
				if ground:getId() == pressedId then
...
for index, value in pairs(decreasing) do tileOut:id(index) end
tileOut:type("stepout")
tileOut:register()