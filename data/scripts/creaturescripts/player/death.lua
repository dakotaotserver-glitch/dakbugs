...
function playerDeath.onDeath(player, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
	if not deathListEnabled then
	 ...
	 -- Remover todos os summons/familiars ao morrer
    for _, sum in ipairs(player:getSummons() or {}) do
        sum:getPosition():sendMagicEffect(CONST_ME_POFF)
        sum:remove()
    end
	...
	Webhook.sendMessage(":skull_crossbones: **" .. player:getName() .. "** has died. Killed at level _" .. player:getLevel() .. "_ by **" .. killerName .. "**.", announcementChannels["player-kills"])
